...
const OperationCompliance = ({ onFinish, onBack, formData, updateFormData }) => {
  ...
  const handleUpdate = (field, value) => {
    // If it's a document, we nest it inside the documents object
    if (field === 'incorporation_cert' || field === 'national_id') {
      updateFormData({
        documents: {
          ...formData.documents,
          [field]: value
    ...
    // Clear errors for this field as the user types/uploads
    if (errors[field]) {
      setErrors(prev => {
  ...
  const handleSubmit = () => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      // Optional: Scroll to top or show a toast if there are errors
  ...
  return (
    <div className="w-full animate-fadeIn pb-4">
      ...
      {/* Business Hours */}
      <div className={`mb-6 flex flex-col md:flex-row items-start md:items-center justify-between p-4 rounded-2xl border transition-all ${errors.opening_time || errors.closing_time ? 'border-red-400 bg-red-50' : 'bg-gray-50 border-gray-100'} gap-3`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        ...
        >
          ...
          <p className="text-[10px] text-gray-400 text-center">
            {formData.documents.incorporation_cert ? formData.documents.incorporation_cert.name : (errors.incorporation_cert || 'PDF or JPG (max 5MB)')}
          </p>
        </div>
        ...
        >
          ...
          <p className="text-[10px] text-gray-400 text-center">
            {formData.documents.national_id ? formData.documents.national_id.name : (errors.national_id || 'PDF or JPG (max 5MB)')}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-4 w-full">
        ...
        <button onClick={handleSubmit} className="flex-1 max-w-[140px] h-11 bg-[#F2B53D] text-white font-bold rounded-full shadow-lg hover:bg-[#e0a630] transition-all text-[14px]">Submit</button>
      </div>
    </div>
  );
};
