...
import {
  Package, ChevronRight, Plus, ListChecks,
  AlertCircle, Star, CreditCard, Box, X,
} from 'lucide-react';
import {
...
const VendorDashboard = () => {
  ...
  const [vendorData, setVendorData] = useState({
    storeName: '', vendorType: '', country: '', businessCategory: '',
    vendor_type: '', is_product_vendor: true, is_service_vendor: false,
    currencySymbol: '', branchLocation: '',
  });
  const [metrics, setMetrics] = useState({
    grossSales: 0, openOrders: 0, pendingPayouts: 0, activeListings: 0,
  });
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        const accurateActiveListings = products.filter((p) => p.is_published === true).length;
        ...
        // salesHistory is already derived in authService (falls back to order totals by date)
        setSalesHistory(response.salesHistory || []);

        setRecentOrders((response.recentOrders || []).slice(0, 7));
        ...
        if (computedAlerts.length === 0 && products.length > 0) {
          computedAlerts = products
            .filter((p) => {
              ...
              const variantLowStock = Array.isArray(p.variants) &&
                p.variants.some((v) => (v.stock ?? 0) <= 5);
              return productStock <= 5 || variantLowStock;
            })
            .map((p) => ({ title: p.title, quantity: p.stock ?? p.detail?.stock ?? 0 }));
        }
    ...
    return (
      <div className="flex min-h-screen bg-[#ecece7] items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#F5B841] mx-auto" />
        ...
        <main className="p-5 max-w-[1400px] mx-auto w-full pb-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="lg:col-span-2 space-y-5">
              ...
              {/* Sales Performance — driven by salesHistory (real orders → date-grouped totals) */}
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="font-bold text-xs uppercase tracking-tighter mb-3">Sales Performance</h3>
                  ...
                  {salesHistory.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={salesHistory}>
                        ...
                        <YAxis hide />
                        <Tooltip
                          formatter={(value) => [`${displayCurrency} ${Number(value).toLocaleString()}`, 'Sales']}
                        />
                        <Area
                          type="monotone"
                          dataKey="sales"
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-slate-100 flex justify-between items-center">
                  ...
                  <Link to="/order-management" className="text-[#125852] text-[9px] font-bold hover:underline">
                    VIEW ALL
                  </Link>
                </div>
                        ...
                        {recentOrders.map((order, i) => (
                          <tr key={order.id ?? i} className="hover:bg-slate-50 transition-colors">
                            ...
                            <td className="px-4 py-2.5 font-bold text-slate-800">
                              {displayCurrency} {Number(order.total || 0).toLocaleString()}
                            </td>
                            <td className="px-4 py-2.5">
            ...
            <div className="space-y-5">
              ...
              <div className="bg-[#125852] p-4 rounded-xl text-white shadow-lg">
                ...
                <h3 className="text-xl font-bold mb-3">
                  {displayCurrency} {(metrics.pendingPayouts || 0).toLocaleString()}
                </h3>
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <Link to="/reviews" className="text-[#125852] text-[9px] font-bold">VIEW ALL</Link>
                </div>
                <div className="space-y-3">
                  {reviews.length > 0 ? (
                    reviews.map((r, i) => (
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden" style={{ fontFamily: "'Poppins', sans-serif" }}>
            <div className="px-5 py-4 border-b border-slate-100 flex justify-between items-center">
              ...
              <button onClick={() => setSelectedOrder(null)} className="p-1.5 rounded-lg hover:bg-slate-100 transition-colors">
            ...
            <div className="p-5 space-y-4 overflow-y-auto max-h-[70vh]">
              ...
              <div className="bg-slate-50 rounded-xl p-4">
                <h3 className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-3">Customer Details</h3>
                <div className="space-y-2">
              ...
              {/* Items */}
              <div className="bg-slate-50 rounded-xl p-4">
                <h3 className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-3">Items Ordered</h3>
                {Array.isArray(selectedOrder.items) && selectedOrder.items.length > 0 ? (
                      ...
                      return (
                        <div key={idx} className="flex justify-between items-start py-2 border-b border-slate-200 last:border-0">
                          <div className="flex-1">
                            ...
                            {itemVariant && <p className="text-[9px] text-slate-400 mt-0.5">Variant: {itemVariant}</p>}
                  ...
                  <div className="flex justify-between items-center py-1.5">
                    <p className="text-[11px] font-bold text-slate-700">Order {formatOrderId(selectedOrder.id)}</p>
              ...
              {/* Payment Summary */}
              <div className="bg-slate-50 rounded-xl p-4">
                <h3 className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-3">Payment Summary</h3>
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
