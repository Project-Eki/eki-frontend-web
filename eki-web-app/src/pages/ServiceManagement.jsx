  ...
  <div className="bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-md transition-all group">
    ...
    <div className="p-3">
      <div className="flex items-center justify-between mb-1">
        <span className="text-[9px] font-bold text-slate-400 tracking-widest truncate max-w-[100px]">
          {s.category}
        </span>
        <span
          className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full uppercase shrink-0 ${STATUS_STYLE[s.status] || STATUS_STYLE.draft}`}
        >
          {s.status}
        </span>
      </div>
      <h3 className="font-black text-[14px] text-slate-900 leading-tight mb-0.5 line-clamp-1">
        {s.title || "—"}
      </h3>
      ...
      <div className="flex items-center justify-between text-[11px] text-slate-400 mb-2">
        <span className="flex items-center gap-0.5">
          <Clock size={11} /> {s.duration || "N/A"}
        </span>
        <span className="flex items-center gap-0.5 font-semibold">
          <Calendar size={11} /> {s.avail || "Available"}
        </span>
      </div>
...
const ServiceManagement = () => {
          ...
          raw.map((item) => {
            ...
            return {
              ...
              duration:
                ...
                (item.detail?.duration_days
                  ? `${item.detail.duration_days} days`
                  : "N/A"),
              avail: item.availability || "Available",
              status: item.status || "draft",
              mode:
                item.detail?.delivery_mode ||
  ...
  const counts = useMemo(
    () => ({
      ...
      published: services.filter(
        (s) => s.status === "published" || s.status === "active",
      ).length,
  ...
  const sorted = useMemo(() => {
    const r = [...filtered];
    if (sortBy === "price_asc")
      r.sort((a, b) => parseFloat(a.price) - parseFloat(b.price));
    if (sortBy === "price_desc")
      r.sort((a, b) => parseFloat(b.price) - parseFloat(a.price));
    if (sortBy === "newest")
      r.sort((a, b) => String(b.id).localeCompare(String(a.id)));
    if (sortBy === "oldest")
      r.sort((a, b) => String(a.id).localeCompare(String(b.id)));
    return r;
  }, [filtered, sortBy]);
            ...
            <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl p-20 text-center">
              ...
              <p className="text-slate-500 max-w-xs mx-auto mt-2 text-sm">
                {search
                  ...
                  : statusFilter !== "all"
                    ? `You have no ${statusFilter} services yet.`
          ...
          {!loading && services.length > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-6 pt-4 border-t border-slate-100">
              <p className="text-sm text-slate-500">
                Showing <b>{sorted.length}</b> of <b>{services.length}</b>{" "}
                services
              </p>
              <div className="flex items-center gap-1">
                <button className="h-9 px-4 border border-slate-200 rounded-lg text-sm text-slate-500 hover:border-[#F5B841] font-medium">
                  Previous
                </button>
                <button className="w-9 h-9 rounded-lg text-sm font-bold bg-[#EFB034FF] text-white">
                  1
                </button>
                <button className="h-9 px-4 bg-[#EFB034FF] text-white rounded-lg text-sm font-bold hover:bg-[#F5B841]">
                  Next
                </button>
              </div>
            </div>
