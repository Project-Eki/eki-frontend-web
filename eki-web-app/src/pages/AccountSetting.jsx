                ...
                <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 text-center">
                  <div className="relative inline-block mb-2">
                    <div className="w-20 h-20 rounded-full mx-auto bg-slate-100 border-2 border-slate-200 overflow-hidden flex items-center justify-center">
                      {userData.profileImage ? <img src={userData.profileImage} className="w-full h-full object-cover" alt="profile" /> : <span className="text-slate-300 text-xs">Photo</span>}
                    </div>
                    <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFileChange} />
                    <button onClick={handleEditPhotoClick} className="absolute bottom-0 right-0 bg-white border border-slate-200 p-1.5 rounded-full text-slate-400 hover:text-slate-600 shadow-sm">
                  ...
                  <form onSubmit={handleSubmit} noValidate className="space-y-4">
                    <div className="flex gap-4">
                      <div className="flex-1 space-y-1">
                        ...
                        <input name="firstName" type="text" value={userData.firstName} onChange={handleChange} className={`w-full px-3 py-2 text-sm border rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-[#F5B841] transition-all ${errors.firstName ? 'border-red-500' : 'border-slate-200'}`} />
                        {errors.firstName && <p className="text-[9px] text-red-500 font-medium">{errors.firstName}</p>}
                      </div>
                      <div className="flex-1 space-y-1">
                        <label className="text-[11px] font-bold uppercase text-slate-500">Last Name</label>
                        <input name="lastName" type="text" value={userData.lastName} onChange={handleChange} className={`w-full px-3 py-2 text-sm border rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-[#F5B841] transition-all ${errors.lastName ? 'border-red-500' : 'border-slate-200'}`} />
                        {errors.lastName && <p className="text-[9px] text-red-500 font-medium">{errors.lastName}</p>}
                      </div>
                    </div>
                    <div className="space-y-1">
                      <label className="text-[11px] font-bold uppercase text-slate-500">Email Address</label>
                      <input name="email" type="email" value={userData.email} onChange={handleChange} className={`w-full px-3 py-2 text-sm border rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-[#F5B841] transition-all ${errors.email ? 'border-red-500' : 'border-slate-200'}`} />
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                      <button type="button" onClick={() => window.location.reload()} className="px-6 py-2 text-[11px] font-bold border border-slate-200 rounded-lg bg-white hover:bg-slate-50 transition-colors">Cancel</button>
                      <button type="submit" disabled={isLoading} className="px-6 py-2 text-[11px] font-bold bg-[#F5B841] text-white rounded-lg shadow-sm hover:bg-[#E0A83B] disabled:opacity-50 transition-all">
                    ...
                    {isDropdownOpen && (
                      <div className="absolute top-full left-0 w-full bg-white mt-2 border border-slate-200 rounded-lg shadow-xl z-[70] overflow-hidden">
                        {availableSocials.map((social) => (
                          <button key={social.name} onClick={() => handleLinkSocial(social)} className="w-full text-left px-4 py-2.5 text-[11px] font-medium text-slate-700 hover:bg-slate-50 flex items-center gap-3 border-b last:border-none transition-colors">
                            <div className="w-2 h-2 rounded-full" style={{ backgroundColor: social.color }}></div>
                            {social.name}
                          </button>
                        ))}
