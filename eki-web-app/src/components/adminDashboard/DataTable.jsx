...
const SEARCH_KEYS_MAP = {
  userManagement: ["email", "created"],
  verificationWorkflows: ["applicant", "submitted", "type"],
};
...
const DataTable = ({
  ...
  // Sync external search into local state
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeSearch.trim()) {
      ...
      const keys = SEARCH_KEYS_MAP[tableType];
  ...
  const handleViewAll = () => {
    const route = VIEW_ALL_ROUTES[tableType] || VIEW_ALL_ROUTES.default;
    if (route !== "#") {
      navigate(route);
    }
  };
          ...
          <thead className="bg-gray-50">
            <tr>
              {columns.map((col, i) => (
                <th
                  key={i}
                  className="px-4 py-2.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap"
                >
                  {col.header}
                </th>
              ))}
