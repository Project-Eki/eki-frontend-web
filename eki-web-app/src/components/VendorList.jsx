...
// Category Cell Component - This is the key change!
const CategoryCell = ({ vendor }) => {
  // For 'both' vendors, show all categories as badges
  if (vendor.businessType === 'both' && vendor.businessCategories && vendor.businessCategories.length > 1) {
    return (
      <div className="flex flex-wrap gap-1">
        {vendor.businessCategories.map((cat, idx) => (
          <span
            key={idx}
            className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-medium bg-purple-50 text-purple-700 border border-purple-100"
          >
            {cat}
          </span>
        ))}
  ...
  // For single category vendors (products or services)
...
const VendorList = ({ vendors, onSelect, selectedId }) => {
  ...
  const filtered = vendors
    ...
    .filter((v) => {
      ...
      // Also search through business_categories array
      const categoriesMatch =
        v.businessCategories?.some((cat) => cat.toLowerCase().includes(q)) ||
        false;
      return (
  ...
  const handleExport = () => {
    ...
    const rows = filtered.map((v) => {
      let categoryDisplay = v.businessCategory || "";
      if (v.businessType === "both" && v.businessCategories?.length > 0) {
        categoryDisplay = v.businessCategories.join(", ");
      }
      return [
        `"${v.name || ""}"`,
        `"${v.email || ""}"`,
