...
//       setIsLoading(true);
//       try {
//         // Send the access_token to your Render backend
//         const data = await googleAuth({
//           access_token: tokenResponse.access_token,
//           requested_role: 'vendor',
//         });

//         // Store tokens for the interceptors in api.js to use
//         if (data.access) localStorage.setItem('access_token', data.access);
//         if (data.refresh) localStorage.setItem('refresh_token', data.refresh);

//         // SUCCESS: Move to the next step
//         // Tip: If the backend says Google users are already verified, 
//         // you might want to call onNext() twice or change currentStep to 3.
//         onNext(); 
        
//       } catch (error) {
//         console.error("Backend Google Auth Error:", error);
//         setErrors({ general: "Could not sync Google account with our server." });
//       } finally {
//         setIsLoading(false);
//       }
//     },
...
//         <button type="button" 
//         onClick={() => loginWithGoogle()}
//         className="w-full h-9 border border-gray-200 rounded-full flex justify-center items-center gap-3 bg-white hover:bg-gray-50 transition font-bold text-[14px] text-gray-700 cursor-pointer">
//           <FcGoogle size={18} /> Sign up with Google
//         </button>
//       </form>
//       <p className="text-center text-[14px] text-gray-500 mt-4">
