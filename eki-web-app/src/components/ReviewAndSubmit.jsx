  ...
  const handleSubmit = async () => {
    ...
    console.log("Documents:", {
      government_issued_id:
        formData.documents?.government_issued_id instanceof File
          ? formData.documents.government_issued_id.name
          : "No file",
      government_issued_id_expiry:
        formData.documents?.government_issued_id_expiry,
      business_license:
        formData.documents?.business_license instanceof File
          ? formData.documents.business_license.name
          : "No file",
      business_license_expiry: formData.documents?.business_license_expiry,
      tax_certificate:
        formData.documents?.tax_certificate instanceof File
          ? formData.documents.tax_certificate.name
          : "No file",
      tax_certificate_expiry: formData.documents?.tax_certificate_expiry,
      incorporation_cert:
        formData.documents?.incorporation_cert instanceof File
          ? formData.documents.incorporation_cert.name
          : "No file",
      professional_body_certification:
        formData.documents?.professional_body_certification instanceof File
          ? "Has file (optional)"
  ...
  const ReviewItem = ({
    ...
    <div className={colSpan ? "col-span-2" : ""}>
      ...
      {children ? (
        ...
        <>
          <span
            className={`font-medium text-[9px] leading-tight block truncate ${
    ...
    <div className="w-full">
      ...
      {Object.keys(fieldErrors).length > 0 && (
        <div className="mb-3 p-2 bg-yellow-50 border border-yellow-200 rounded-md">
          <p className="text-yellow-700 text-[9px] font-medium mb-1">
      ...
      <div className="space-y-0">
        ...
        <ReviewSection title="BUSINESS IDENTITY">
          ...
          <ReviewItem
            label="Description"
            value={formData.business_description?.substring(0, 40)}
            colSpan
          ...
          <ReviewItem label="Country" fieldName="country">
            <div className="flex items-center gap-0.5 mt-0.5">
              <Flag
                ...
                className="w-2.5 h-1.5 rounded-sm object-cover shadow-sm"
              />
              <span
                className={`font-medium text-[9px] ${fieldErrors.country ? "text-red-600" : "text-gray-800"}`}
              >
                {countryName}
              </span>
              {fieldErrors.country && (
          ...
          <ReviewSection title="BRANCH LOCATIONS">
            <div className="col-span-2 space-y-1">
              {formData.branch_locations.map((branch, idx) => (
                <div
                  key={idx}
                  className="text-[9px] text-gray-700 border-l-2 border-[#F2B53D] pl-2"
                >
        ...
        <div className="pt-1.5 pb-0.5">
          ...
          <div className="flex flex-wrap gap-0.5">
            {documents.length === 0 ? (
              <span className="text-gray-400 text-[8px] italic">
              ...
              documents.map(([key]) => (
                <div
                  key={key}
                  className="flex items-center gap-0.5 bg-green-50 text-green-700 px-1 py-0.5 rounded border border-green-100 text-[7px] font-semibold"
                >
