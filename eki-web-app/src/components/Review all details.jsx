import { HiOutlinePencilAlt, HiCheckCircle, HiX } from "react-icons/hi";
...
const ReviewPhase = ({ formData, onEdit, onSubmit, isLoading }) => {
  ...
  const ReviewSection = ({ title, children }) => (
    <div className="border-b border-gray-100 pb-4 last:border-0 pt-4 first:pt-0">
      <p className="text-[10px] font-black text-[#F2B53D] uppercase tracking-widest mb-3">{title}</p>
      <div className="grid grid-cols-2 gap-4 text-[13px]">{children}</div>
    </div>
  );

  const ReviewItem = ({ label, value, colSpan = false }) => (
    <div className={colSpan ? "col-span-2" : ""}>
      <p className="text-gray-400 block mb-0.5">{label}</p>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal Content */}
      <div className="relative w-full max-w-2xl bg-white rounded-[32px] shadow-2xl overflow-hidden animate-slideUp flex flex-col max-h-[90vh]">
        
        {/* Sticky Header */}
        <div className="bg-gray-50 p-6 flex justify-between items-center border-b border-gray-100 shrink-0">
          <div>
            <h4 className="font-black text-[22px] text-gray-900 leading-none">Final Review</h4>
            <p className="text-[14px] text-gray-500 font-medium mt-1">Verify your details before submission</p>
          </div>
          <button
            onClick={onEdit}
            disabled={isLoading}
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
          ...
          <ReviewSection title="Contact & Location">
            ...
            <ReviewItem label="Full Address" value={fullAddress} colSpan />
          </ReviewSection>
          ...
          <ReviewSection title="Operations & Docs">
            ...
            <div className="col-span-2 mt-2">
              <p className="text-gray-400 mb-2">Files Attached</p>
              {documents.length === 0 ? (
                <span className="text-gray-400 text-[11px]">No documents uploaded</span>
              ) : (
                <div className="flex flex-wrap gap-2">
                  {documents.map(([key]) => (
                    <div key={key} className="flex items-center gap-2 bg-green-50 text-green-700 px-3 py-1.5 rounded-lg border border-green-100 text-[11px] font-bold uppercase">
        ...
        <div className="p-6 bg-white border-t border-gray-100 shrink-0">
          <div className="flex gap-4">
            <button
              onClick={onEdit}
              disabled={isLoading}
              className="flex-1 h-12 border-2 border-gray-100 text-gray-500 font-bold rounded-2xl hover:bg-gray-50 transition-all"
            >
              Edit Details
            </button>
            <button
              onClick={onSubmit}
              disabled={isLoading}
              className="flex-[1] h-12 bg-[#F2B53D] hover:bg-[#d9a236] text-white font-bold rounded-2xl shadow-lg shadow-orange-100 transition-all flex items-center justify-center gap-3 disabled:bg-gray-300 disabled:shadow-none"
            >
              {isLoading ? (
